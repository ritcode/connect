...
let isMorebuttonsVisible = false;
...
// more Buttons
let moreButtons;
let moreButtonsHeader;
let tabDevicesBtn;
let tabBandwidthBtn;
let tabRoomBtn;
// let tabThemeBtn;
let mySettingsCloseBtn;
let moreButtonsCloseBtn;
let myPeerNameSet;
...
function getHtmlElementsById() {
  ...
  screenFpsLabel = getId("screenfps-label");
  ...
  //more buttons
  moreButtons = getId("moreButtons");
  moreButtonsHeader = getId("moreButtonsHeader");
  moreButtonsCloseBtn = getId("moreButtonsCloseBtn");
  // my whiteboard
...
function initPeer() {
  ...
  signalingSocket.on("unauthorized", handleUnauthorizedKick)
 ...
 * Once the user has given access to their
 * microphone/camcorder, join the channel
 * and starts peering up
 */
...
function welcomeUser() {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      let message = {
        email: "",
        subject: "Please join My Connect Meeting",
...
/**
 * When user joins a group, signaling server will send out 'addPeer' events to each pair
...
/** Function to handle on ICE candidate
 * 
...
/** Function to handle on media tracks from remote peer
 * 
    ...
    event.channel.onmessage = (msg) => {
      switch (event.channel.label) {
        case "connect_message_channel":
          ...
          break;
        case "connect_file_sharing_channel":
 ...
 * attach it to an <audio> or <video> tag if they give access.
...
function setChatRoomBtn() {
  ...
  // close chat room - show main button and status menu if hide
  ...
  msgerSendBtn.addEventListener("click", (e) => {
    ...
    if (!thereIsPeerConnections()) {
      userLog("info", "Can't send message, No peer detected");
      msgerInput.value = "";
      return;
    }
...
/**
 * more buttons click event
 */
function setMoreBtn() {
  moreBtn.addEventListener("click", (e) => {
    hideShowMoreButtons();
  });
  moreButtonsCloseBtn.addEventListener("click", (e) => {
    hideShowMoreButtons();
  });
  //make it draggable
  dragElement(moreButtons, moreButtonsHeader);
}

/**
 * Handle main buttons - status menu show - hide on body mouse move
 */
...
function setupMySettings() {
  ...
  if (isMobileDevice) {
    ...
    screenFpsSelect.style.display = "none";
    screenFpsLabel.style.display = "none";
  }
  // room actions
...
/**
 * Show main buttons & status menu for 10 seconds on body mousemove
 * if mobile and chatroom open do nothing return
 * if mobile and mySettings open do nothing return
 */
function showMainButtonsAndMenu() {
  if (
    isButtonsVisible ||
...
async function shareRoomUrl() {
  ...
  if (navigator.share) {
    ...
    try {
      // can add title and description to load metadata from url
  ...
  if (
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let message = {
          email: "",
          subject: "Please join Connect Meeting",
          body: "Click to join: " + myRoomUrl,
        };
        shareRoomByEmail(message);
      }
...
function refreshMyLocalStream(stream, localAudioTrackChange = false) {
  ...
  /** when user stop the screen sharing, on default it turns back to the webcam with video stream ON.
 ...
 * It can Record One Media Stream at time
...
function setChatRoomForMobile() {
  if (isMobileDevice) {
    document.documentElement.style.setProperty("--msger-height", "90%");
    document.documentElement.style.setProperty("--msger-width", "90%");
  } else {
 ...
 */
 function hideShowMoreButtons() {
  if (!isMorebuttonsVisible) {
    if (isMobileDevice) {
      moreButtons.style.setProperty("width", "max-content");
    }
    // center screen on show
    moreButtons.style.top = "50%";
    moreButtons.style.left = "50%";
    moreButtons.style.display = "block";
    isMorebuttonsVisible = true;
    return;
  }
...
/**
 * Unauthorized Kick handler
 */
function handleUnauthorizedKick() {
  Swal.fire({
    background: swalBackground,
    position: "center",
