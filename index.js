...
let PORT = process.env.PORT || 5000; // signalingServerPort
let localHost = "http://localhost:" + PORT; 

let channels = {}; // collect channels
let sockets = {}; // collect sockets
let peers = {}; // collect peers info grp by channels
let admins = {}; //collect admins of channels
...
// all start from public/index.html
...
/**
 * Should probably use a different stun-turn server
...
server.listen(PORT, null, () => {
  addLog("Signal server started");
    // server settings
    addLog(" with settings", {
...
io.sockets.on("connect", (socket) => {
  addLog("[" + socket.id + "] --> connection accepted");
  ...
  socket.on("join", (config) => {
    ...
    if (channel in socket.channels) {
      addLog("[" + socket.id + "] [Warning] already joined", channel);
      return;
    }
    // no channel aka room in channels init
    if (!(channel in channels)) {
      channels[channel] = {};
      admins[channel] = socket.id;
    }
    ...
    addLog("connected peers grp by roomId", peers);
    ...
    for (let id in channels[channel]) {
      ...
      addLog("[" + socket.id + "] emit addPeer [" + id + "]");
    }
  ...
  async function removePeerFrom(channel) {
    if (!(channel in socket.channels)) {
      addLog("[" + socket.id + "] [Warning] not in ", channel);
      return;
    }
    ...
    for (let id in channels[channel]) {
      ...
      addLog("[" + socket.id + "] emit removePeer [" + id + "]");
    }
  }
  ...
  socket.on("peerName", (config) => {
    ...
    for (let peer_id in peers[room_id]) {
      if (peers[room_id][peer_id]["peer_name"] == peer_name_old) {
        ...
        addLog("[" + socket.id + "] change peer name", {
          room_id: room_id,
          peer_id: peer_id,
    ...
    if (peer_id_to_update && Object.keys(peerConnections).length != 0) {
      addLog("[" + socket.id + "] emit peerName to [room_id: " + room_id + "]", {
  ...
  socket.on("peerStatus", (config) => {
    ...
    for (let peer_id in peers[room_id]) {
      if (peers[room_id][peer_id]["peer_name"] == peer_name) {
        ...
        addLog("[" + socket.id + "] change " + element + " status", {
          room_id: room_id,
          peer_name: peer_name,
          element: element,
          status: status,
  ...
  socket.on("fileInfo", (config) => {
    ...
    /**
    *  Calculate Human readable size format
    */
  ...
  socket.on("wb", (config) => {
    ...
    if (Object.keys(peerConnections).length != 0) {
       addLog("[" + socket.id + "] whiteboard config", config);
      for (let peer_id in peerConnections) {
        if (sockets[peer_id]) {
